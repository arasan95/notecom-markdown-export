...
function run(html, url) {
  ...
  const downloads = [];
  dom.window.GM_download = (o) => downloads.push({ ...o });
...
const r = run(html, 'https://note.com/taro/n/na1b2c3d4e5f6');
console.log('status:', r.status);
console.log('imgs:', r.imgs.map(i => i.name).join(', '));
...
if (!r.imgs.length || r.imgs[0].name !== 'notecom/テスト記事-image-1.png') {
  console.log('NG 画像保存先: ' + (r.imgs[0] && r.imgs[0].name));
  ok = false;
} else {
  console.log('OK 画像保存先 notecom/テスト記事-image-1.png');
}
...
const checks = [
  ...
  [/^1\.\s+手順A$/m, 'ol'],
  [/\| 列 \|/, 'テーブル'],
  [/!\[スクショ\]\(テスト記事-image-1\.png\)/, 'data-src画像'],
];
let ok = true;
...
const r3 = run(html2, 'https://note.com/taro/n/na1b2c3d4e5f7');
...
// エラーパス: 本文なし
try {
  const r2 = run('<!DOCTYPE html><html><body><h1>a</h1></body></html>', 'https://note.com/x/n/n123456789abc');
  console.log('error path status:', r2.status);
  if (!/エラー/.test(r2.status)) { console.log('NG エラーパス'); ok = false; } else console.log('OK エラーパス');
} catch (e) { console.log('NG エラーパス throw:', e.message); ok = false; }
